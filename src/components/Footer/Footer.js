...
const Footer = ({ changeShowContent, showContent }) => {
    ...
    const handleClearComplete = () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        yield todos.filter(todo => todo.isComplete).forEach(todo => {
            try {
                return pages_1.deleteTodoAction(todo.id);
            }
            catch (e) {
                console.log(e);
            }
        });
        yield reducers_1.clearComplete();
    });
    return (react_1.default.createElement("footer", { className: 'footer' },
        ...
        react_1.default.createElement("button", { "data-testid": 'button', className: 'clear-completed', style: { visibility: shared_1.checkAnyComplete(showContent) ? "visible" : "hidden" }, onClick: handleClearComplete }, "Clear completed")));
};
